...
watchEffect(() => {
    setLayoutProps({
        breadcrumbs: [{ title: t('nav.camps'), href: '/camps' }],
    });
});
...
<template>
    <Head :title="t('nav.camps')" />

    <div class="flex h-full flex-1 flex-col gap-6 p-4">
        <div class="flex items-start justify-between gap-4">
            <Heading
                :title="t('nav.camps')"
                :description="t('camps.index.description')"
            />
            <Button as-child>
        ...
        >
            <Link
                v-for="camp in camps"
                :key="camp.id"
                :href="show(camp.id).url"
                class="group"
            >
                <Card
                    class="h-full py-0 transition-colors group-hover:border-primary/50"
                >
                    <CardContent class="flex h-full flex-col gap-3 p-5">
                        ...
                        <div
                            class="mt-auto flex items-center gap-4 pt-2 text-sm text-muted-foreground"
                        >
        ...
        <div v-else class="rounded-xl border border-dashed p-12 text-center">
            ...
            <h3 class="mt-4 font-medium">{{ t('camps.index.empty') }}</h3>
            <p class="mt-1 text-sm text-muted-foreground">
                {{ t('camps.index.emptyHint') }}
            </p>
