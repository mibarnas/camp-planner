...
function remove(slot: TimeSlot) {
    if (
        !confirm(
            `Odstrániť blok „${slot.name}"? Zmažú sa aj jeho záznamy vo všetkých dňoch.`,
        )
    ) {
        return;
    }
    ...
    <div class="grid gap-4 lg:grid-cols-[1fr_1.1fr]">
        <div class="grid content-start gap-2">
            ...
            >
                ...
                <div v-if="editable" class="flex shrink-0 gap-1">
                    <Button
                        variant="ghost"
                        size="icon-sm"
        ...
        >
            <p class="text-sm font-medium">
                {{ editingId ? 'Upraviť blok' : 'Pridať blok' }}
            </p>
            ...
            <div class="grid gap-2 sm:grid-cols-2">
                <div class="grid gap-2">
                    ...
                    <Select
                        ...
                        @update:model-value="
                            form.kind = $event as 'fixed' | 'activity'
                        "
                    >
