...
function revokeShareLink() {
    ...
    if (
        !confirm(
            'Zrušiť zdieľateľný odkaz? Existujúci odkaz prestane fungovať.',
        )
    ) {
        return;
    }
...
function removeMember(member: CampMember) {
    ...
    router.delete(destroyMember({ camp: props.campId, user: member.id }).url, {
        preserveScroll: true,
    });
}
...
function cancelInvitation(invitation: CampInvitation) {
    router.delete(
        destroyInvitation({ camp: props.campId, invitation: invitation.id })
            .url,
        {
            preserveScroll: true,
        },
        ...
        >
            ...
            <template v-if="shareLink">
                ...
                <p class="text-xs text-muted-foreground">
                    Ktokoľvek s odkazom sa môže pridať ako vedúci. Odkaz funguje
                    opakovane.
                </p>
            </template>
            <template v-else>
                ...
                <p class="text-xs text-muted-foreground">
                    Vytvorí odkaz na pripojenie bez pozvánky e-mailom — pre
                    neobmedzený počet ľudí.
                </p>
            </template>
        </div>
        ...
        <div v-if="invitations.length" class="grid gap-2">
            ...
            >
                ...
                <div class="flex items-center gap-1">
                    ...
                    >
                        <component
                            :is="copiedId === invitation.id ? Check : Copy"
                        />
                        {{
