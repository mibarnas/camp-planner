...
const dayQuestions = computed(() =>
    props.questions.filter((q) => q.scope === 'day'),
);
...
// Only real programme gets rated — nobody needs to score breakfast out of five.
...
const steps = computed<Step[]>(() => {
    const list: Step[] = items.value.map((entry) => ({
        kind: 'activity',
        entry,
    }));
...
const ratedCount = computed(
    () => Object.values(form.ratings).filter((r) => r.rating > 0).length,
);
...
function submit() {
    form.transform((data) => ({
        ...
        camp_reason:
            data.camp_rating && data.camp_rating < 5 ? data.camp_reason : null,
...
<template>
    <Dialog :open="open" @update:open="emit('update:open', $event)">
        ...
        >
            ...
            <!-- Header -->
            <div
                class="flex shrink-0 items-center justify-between px-4 pt-3 pb-1"
            >
            ...
            >
                <Transition
                    :name="dir > 0 ? 'slide-next' : 'slide-prev'"
                    mode="out-in"
                >
                    <div
                        :key="index"
                        class="flex h-full flex-col overflow-y-auto px-6 py-4"
                    >
                        <!-- ACTIVITY -->
                        <template v-if="current?.kind === 'activity'">
                            ...
                            >
                                ...
                                <p
                                    class="text-xs font-medium tracking-wide text-white/50 uppercase"
                                >
                                    Aktivita {{ index + 1 }} /
                                    {{ items.length }}
                                </p>
                                <h2
                                    class="mt-1 text-2xl leading-tight font-bold text-balance"
                                >
                                    {{
                                        current.entry.title ||
                                        current.entry.activity?.name ||
                                        'Aktivita'
                                    }}
                                </h2>
                                <p class="mt-1 text-sm text-white/60">
                                    {{ current.entry.start_time }} ·
                                    {{ durationLabel(current.entry.duration) }}
                                </p>

                                <div class="mt-8 flex justify-center">
                                    <StarRating
                                        v-model="
                                            form.ratings[current.entry.id]
                                                .rating
                                        "
                                        size="size-11"
                                        gap="gap-1.5"
                                ...
                                <p class="mt-2 h-4 text-xs text-white/40">
                                    {{
                                        form.ratings[current.entry.id].rating
                                            ? `${form.ratings[current.entry.id].rating} / 5`
                                            : t('review.tapStars')
                                    }}
                                </p>
                        ...
                        <template v-else-if="current?.kind === 'question'">
                            <div class="flex flex-1 flex-col justify-center">
                                <div class="mb-5 flex justify-center">
                                    <div
                                        class="flex size-16 items-center justify-center rounded-2xl bg-white/10"
                                    >
                                        <MessageCircleQuestion
                                            class="size-8 text-sky-400"
                                        />
                                    </div>
                                </div>
                                ...
                                <p
                                    class="mt-1 mb-4 text-center text-sm text-white/60"
                                >
                                    {{
                                        current.question.scope === 'camp'
                                            ? t('review.campQuestion')
                                            : t('common.optional')
                                    }}
                                </p>
                                <Textarea
                        ...
                        <template v-else-if="current?.kind === 'notes'">
                            <div class="flex flex-1 flex-col justify-center">
                                ...
                                <p
                                    class="mt-1 mb-4 text-center text-sm text-white/60"
                                >
                                    {{ t('review.dayNotesHint') }}
                                </p>
                                <Textarea
                                    v-model="form.notes"
                                    class="min-h-40 border-white/15 bg-white/5 text-sm text-white placeholder:text-white/40"
                                    :placeholder="
                                        t('review.dayNotesPlaceholder')
                                    "
                                />
                            </div>
                        </template>
                        ...
                        <template v-else-if="current?.kind === 'camp'">
                            ...
                            >
                                <div
                                    class="mb-5 flex size-20 items-center justify-center rounded-3xl bg-primary/20 text-primary shadow-lg"
                                >
                                    <Tent class="size-9" />
                                </div>
                                <p
                                ...
                                <h2 class="mt-1 text-2xl font-bold">
                                    {{ t('review.campOverall') }}
                                </h2>
                                <div class="mt-8 flex justify-center">
                                    <StarRating
                                        v-model="form.camp_rating"
                                        size="size-11"
                                        gap="gap-1.5"
                                    />
                                </div>
                                ...
                                <Transition name="fade">
                                    <Textarea
                                        ...
                                        class="mt-4 min-h-20 w-full border-white/15 bg-white/5 text-sm text-white placeholder:text-white/40"
                                        :placeholder="
                                            t('review.campImprovePlaceholder')
                                        "
                                    />
                                </Transition>
                            </div>
            ...
            <!-- Footer -->
            <div
                class="flex shrink-0 items-center justify-between gap-3 border-t border-white/10 px-4 py-3"
            >
