...
function zoomBy(factor: number, anchorClientX?: number) {
    ...
    if (el && anchorClientX !== undefined) {
        ...
        const contentX =
            el.scrollLeft + (anchorClientX - rect.left) - labelW.value;
        const ratio = next / zoom.value;
        zoom.value = next;
        requestAnimationFrame(() => {
            el.scrollLeft =
                contentX * ratio - (anchorClientX - rect.left) + labelW.value;
        });
    } else {
        zoom.value = next;
    }
...
const pxPerMin = computed(
    () =>
        Math.max(
            MIN_PX,
            (containerW.value - labelW.value - 2) / totalMin.value,
        ) * zoom.value,
);
...
const stripW = computed(() =>
    Math.min(containerW.value, labelW.value + trackWidth.value),
);
...
function cardTooltip(entry: ProgramEntry): string {
    ...
    const lines = [
        entry.title || entry.activity?.name || t('activities.one'),
        `${entry.start_time}–${end} · ${durationLabel(entry.duration)}`,
    ];

    if (entry.responsible) {
        lines.push(t('entry.responsibleLine', { name: entry.responsible }));
    }

    if (entry.notes) {
        lines.push(t('entry.noteLine', { note: entry.notes }));
    }
...
const STATUS_LABELS: Record<EntryStatus, string> = {
    todo: t('entry.status.todoLower'),
    none: t('entry.status.noneLower'),
    done: t('entry.status.doneLower'),
};
const OTHER_STATUSES: EntryStatus[] = ['done', 'none', 'todo'];

function statusTitle(entry: ProgramEntry): string {
    return (
        t('entry.statusLine', { status: STATUS_LABELS[entry.status] }) +
        (props.editable ? t('entry.statusToggleHint') : '')
    );
}
// A quick tap only flips between done and in-progress; "needs sorting out" is
...
function deleteSelected() {
    ...
    if (
        !confirm(
            t('timeline.confirmDeleteSelected', {
...
function openTrackMenu(e: MouseEvent, day: CampDay) {
    ...
    const min = snap(
        bounds.value.start + (e.clientX - rect.left) / pxPerMin.value,
    );
    contextMenu.value = {
        x: e.clientX,
        y: e.clientY,
...
function removeLocalOverride(day: CampDay, slotId: number) {
    const i =
        day.slot_overrides?.findIndex((o) => o.time_slot_id === slotId) ?? -1;
...
function slotTooltip(slot: EffectiveSlot): string {
    ...
    if (slot.hidden) {
        lines.push(t('timeline.slotHidden'));
    } else if (props.editable) {
        lines.push(t('timeline.slotDragHint'));
    }
    ...
    if (slot.overridden) {
        ...
        lines.push(
            template
                ? t('timeline.slotOverriddenTemplate', {
                      start: template.start_time,
                      end: template.end_time,
                  })
                : t('timeline.slotOverridden'),
        );
    }
...
function onPointerMove(e: PointerEvent) {
    ...
    if (d.mode === 'move') {
        ...
        d.entry.duration = Math.max(
            SNAP,
            Math.min(maxDur, d.origDur + deltaMin),
        );
    }
}
...
function beginDrag(e: PointerEvent, entry: ProgramEntry, day: CampDay) {
    ...
    const mode = (e.target as HTMLElement).closest('[data-resize]')
        ? 'resize'
        : 'move';
    ...
    const groupEntries =
        mode === 'move' && isSelected(entry)
            ...
            : [entry];
    const group = groupEntries.map((g) => ({
        entry: g,
        origStart: timeToMin(g.start_time),
    }));
    ...
    for (const g of group) {
        ...
        maxDelta = Math.min(
            maxDelta,
            bounds.value.end - g.entry.duration - g.origStart,
        );
    }
...
function beginSlotDrag(e: PointerEvent, slot: EffectiveSlot, day: CampDay) {
    ...
    const mode = (e.target as HTMLElement).closest('[data-slotresize]')
        ? 'resize'
        : 'move';
...
<template>
    <div class="flex flex-col gap-2">
            ...
            <div class="ml-auto flex flex-wrap items-center gap-2">
                <div class="flex items-center gap-1">
                    <Input
                        ...
                        :placeholder="t('timeline.bulkResponsible')"
                    ...
                    <Button
                        size="sm"
                        variant="outline"
                        @click="applyBulkResponsible"
                    >
                        <UserCheck /> {{ t('timeline.assign') }}
                    </Button>
                </div>
                <Button
                    size="sm"
        ...
        <div
            ref="wrapEl"
            class="overflow-x-auto rounded-xl border bg-card select-none"
        >
            <div :style="{ minWidth: labelW + trackWidth + 'px' }">
                <!-- Header: hour ruler + block guides (the camp-wide template) -->
                <div
                    class="sticky top-0 z-30 flex border-b bg-muted/80 backdrop-blur"
                >
                    <div
                        v-if="!isMobile"
                    ...
                    >
                        ...
                        >
                            <span
                                class="truncate text-[11px] leading-none font-semibold"
                                :class="colorStyle(slot.color).header"
                            >
                                {{ slot.name }}
                            </span>
                            <span
                                class="truncate text-[10px] leading-none text-muted-foreground"
                            >
                ...
                >
                    ...
                    >
                        <div class="flex items-center justify-between gap-2">
                            <p
                                class="min-w-0 truncate leading-tight font-semibold"
                            >
                                ...
                                <span
                                    ...
                                    :title="
                                        t('timeline.avgRating', {
                                            avg: row.day.review_summary.avg,
                                            count: row.day.review_summary
                                                .reviewers,
                                        })
                                    "
                                >
                        ...
                        >
                            ...
                            >
                                <MapPin class="size-3 shrink-0" />{{
                                    row.day.trip_name || t('camps.create.trip')
                                }}
                            </span>
                            <span
                                v-if="row.day.name_days"
                            ...
                            <span
                                v-if="row.day.birthdays"
                                class="text-muted-foreground"
                                >🎂 {{ row.day.birthdays }}</span
                            >
                            <span
                    ...
                    <div class="flex">
                        ...
                        >
                            <div class="flex items-start justify-between gap-1">
                                ...
                                <div
                                    class="flex gap-0.5 opacity-0 transition group-hover/day:opacity-100 pointer-coarse:opacity-100"
                                >
                                    <button
                            ...
                            <p
                                v-if="row.day.name_days"
                                class="mt-1 text-[11px] leading-tight text-muted-foreground"
                            >
                                <span class="font-medium">Meniny:</span>
                                {{ row.day.name_days }}
                            </p>
                            <p
                                v-if="row.day.birthdays"
                        ...
                        >
                            ...
                            <div
                                ...
                                :style="{
                                    top: BAR_H + 'px',
                                    left:
                                        xFor(timeToMin(slot.start_time)) + 'px',
                                    width:
                                        wFor(
                                            timeToMin(slot.end_time) -
                                                timeToMin(slot.start_time),
                            ...
                            >
                                <div
                                    ...
                                    :style="{
                                        left:
                                            xFor(timeToMin(slot.start_time)) +
                                            'px',
                                        width:
                                            Math.max(
                                                10,
                                    ...
                                    @pointerdown="
                                        beginSlotDrag($event, slot, row.day)
                                    "
                                    @contextmenu="
                                        openSlotMenu($event, row.day, slot)
                                    "
                                >
                                    <EyeOff
                                        v-if="slot.hidden"
                                        class="mr-0.5 size-2.5 shrink-0"
                                    />
                                    <span
                                    ...
                                    <div
                                        ...
                                        :title="t('timeline.resizeSlotHint')"
                                    />
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                :class="[
                                    colorStyle(cardColor(item.entry, row.slots))
                                        .cell,
                                    item.entry.kind === 'simple'
                                        ? 'border-l-2 border-dashed opacity-80'
                                        : 'border-l-4',
                                    editable
                                        ? 'cursor-grab active:cursor-grabbing'
                                        : 'cursor-pointer',
                                ...
                                :style="{
                                    left:
                                        xFor(timeToMin(item.entry.start_time)) +
                                        'px',
                                    width:
                                        Math.max(
                                            24,
                                            wFor(item.entry.duration) - 2,
                                        ) + 'px',
                                    top:
                                        BAR_H + PAD + item.lane * LANE_H + 'px',
                                    height: LANE_H - CARD_GAP + 'px',
                                    touchAction: editable ? 'none' : 'auto',
                                }"
                                :title="cardTooltip(item.entry)"
                                @pointerdown="
                                    beginDrag($event, item.entry, row.day)
                                "
                                @contextmenu="
                                    openCardMenu($event, row.day, item.entry)
                                "
                                @click.stop="onCardClick(row.day, item.entry)"
                            >
                                <div
                                    class="flex items-start justify-between gap-1 px-1.5 pt-1"
                                >
                                    <span
                                    ...
                                    >
                                        <Coffee
                                            v-if="item.entry.kind === 'simple'"
                                            class="size-3 shrink-0"
                                        />
                                        <span class="truncate">
                                            {{
                                                item.entry.title ||
                                                item.entry.activity?.name ||
                                                'Aktivita'
                                            }}
                                        </span>
                                    </span>
                                    <span
                                        ...
                                        @click.stop="
                                            editable && cycleStatus(item.entry)
                                        "
                                        @pointerdown.stop
                                    >
                                        <Check
                                            v-if="item.entry.status === 'done'"
                                            class="size-3"
                                        />
                                        <span
                                            v-else-if="
                                ...
                                <div
                                    class="flex items-center gap-1 truncate px-1.5 text-[10px] text-muted-foreground"
                                >
                                    <span class="truncate">
                                        {{ item.entry.start_time }} ·
                                        {{ durationLabel(item.entry.duration) }}
                                    </span>
                                    <StickyNote
                                    ...
                                    <Trophy
                                        ...
                                        class="size-3 shrink-0 text-amber-600 dark:text-amber-400"
                                        :title="t('timeline.scoringActivity')"
                                    />
                                    <span
                                        v-if="item.entry.avg_rating != null"
                                        class="ml-auto flex shrink-0 items-center gap-0.5 font-medium text-amber-500"
                                        :title="
                                            t('timeline.ratingCount', {
                                                count: item.entry.rating_count,
                                            })
                                        "
                                    >
                                        <Star
                                            class="size-2.5 fill-amber-400 text-amber-400"
                                        />{{ item.entry.avg_rating }}
                                    </span>
                                </div>
                                <span
        ...
        >
            <template v-if="contextMenu.entry">
                ...
                <button
                    class="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent pointer-coarse:py-2.5"
                    @click="menuToggleSelect"
                >
                    <CheckSquare class="size-4" />
                    {{
                        isSelected(contextMenu.entry)
                            ? t('timeline.deselect')
                            : t('timeline.select')
                    }}
                </button>
                ...
                <button
                    v-for="s in OTHER_STATUSES.filter(
                        (s) => s !== contextMenu!.entry!.status,
                    )"
                    :key="s"
                    class="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent pointer-coarse:py-2.5"
                ...
                >
                    <Check class="size-4" />
                    {{ t('timeline.markAs', { status: STATUS_LABELS[s] }) }}
                </button>
                <div class="my-1 h-px bg-border" />
                <button
                ...
                >
                    ...
                    {{
                        selectedIds.size > 1 &&
                        selectedIds.has(contextMenu.entry.id)
            ...
            <template v-else-if="contextMenu.slot">
                ...
                >
                    ...
                    {{ contextMenu.slot.start_time }}–{{
                        contextMenu.slot.end_time
                    }}
                </p>
                <button
                ...
                >
                    <Eye class="size-4" /> {{ t('timeline.showThisDay') }}
                </button>
                <button
                    v-else
                    class="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm hover:bg-accent pointer-coarse:py-2.5"
                ...
                >
                    <EyeOff class="size-4" /> {{ t('timeline.hideThisDay') }}
                </button>
                <button
            ...
            <template v-else>
                ...
                >
                    ...
                    {{
                        t('timeline.addAt', {
                            time: minToTime(contextMenu.startMin),
                        })
                    }}
                </button>
            </template>
